...
class Data {
    ...
    //returns an address balance
    func getBalanceFromCoin(coinTicker: String, address: String) -> Int {
        var balance:Int = 0
        
        
        let urlString:String = "https://api.blockcypher.com/v1/\(coinTicker.lowercaseString)/main/addrs/\(address)?token=53fadb28f590427e8854197595feb95a"
        print(urlString)
    ...
    //returns total number of transactions from an address
    func getTransactionNumberFromCoin(coinTicker: String, address: String) -> Int {
        ...
        if let url = NSURL(string: urlString){
            if let data = try? NSData(contentsOfURL: url, options: []){
                let json = JSON(data: data)
                transactionNumber = json["final_n_tx"].intValue
            }
    ...
    //returns information from a transaction at the passed place value in the array
    func getTransactionInfoFromCoinAndPlace(coinTicker: String, address: String, transactionNumber: Int) -> (Bool, Int){
        var (wasSent,amount):(Bool,Int) = (true,0)
        ...
        if let url = NSURL(string: urlString){
            if let data = try? NSData(contentsOfURL: url, options: []){
                ...
                let sentStatus: Int = json["txrefs"].arrayValue[transactionNumber]["tx_input_n"].intValue
                ...
                amount = json["txrefs"].arrayValue[transactionNumber]["value"].intValue
                
            }
            
        }

        return (wasSent,amount)
    }
